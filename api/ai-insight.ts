...
export default async function handler(request: { method?: string; body?: RequestBody }, response: { status: (code: number) => { json: (data: unknown) => void }; json: (data: unknown) => void }) {
  ...
  try {
    input = typeof request.body === 'string' ? (JSON.parse(request.body) as RequestBody) : (request.body || {})
  } catch {
    input = {}
  }
  ...
  for (const model of models) {
    ...
    if (upstream.ok) {
      ...
      response.status(200).json({ text: data.candidates?.[0]?.content?.parts?.[0]?.text || 'No insight was returned.' })
      return
    }
  }
